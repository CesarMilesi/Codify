const appChangeLanguage = Vue.createApp({
  data() {
    return {
      // Retrieve selected language from local storage, otherwise defaulting to 'en'
  ...
  methods: {
    ...
    */
    async fetchJsonContent(pageName) {
    ...
    updateContent() {
      if (this.translations) {
        ...
        document.querySelectorAll('[data-i18n]').forEach(element => {
          ...
          const translation = this.translations[this.selectedLanguage]?.[key] || key;

          // Check for missing data-i18n keys
          if (!this.translations[this.selectedLanguage]?.[key]) {
            console.warn(`data-i18n key '${key}' does not have an equivalent in the JSON file.`);
          }
          element.innerHTML = translation;
        });
      // Check for missing keys in JSON files
      const languageTranslations = this.translations[this.selectedLanguage];
      Object.keys(languageTranslations).forEach(key => {
        if (!document.querySelector(`[data-i18n="${key}"]`)) {
          console.warn(`Key '${key}' in language '${this.selectedLanguage}' JSON file does not have an equivalent data-i18n in the HTML.`);
        }
      });
      }
    }
...
appChangeLanguage.mount('#appChangeLanguage');
